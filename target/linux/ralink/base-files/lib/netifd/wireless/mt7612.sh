...
drv_mt7612_init_device_config() { 
	...
	config_add_boolean greenap diversity noscan smart
...
mt7612_ap_vif_post_config() {
	...
	json_select config
	json_get_vars disabled encryption key key1 key2 key3 key4 ssid mode wps pin isolated doth hidden rssikick rssiassoc
	json_select ..
...
mt7612_sta_vif_connect() {
	...
	[ "$ApIfCNT" == "0" ] &&
	{
		...
		ifconfig rai0 up
		ifconfig apclii0 up
		iwpriv rai0 set DisConnectAllSta=1 2>/dev/null
		ifconfig rai0 down
	}
	let stacount+=1
	
	killall  $APCLI_5G_APCTRL
	[ ! -z "$key" ] && APCTRL_KEY_ARG="-k"
	[ ! -z "$bssid" ] && APCTRL_BSS_ARG="-b $(echo $bssid | tr 'A-Z' 'a-z')"
	$APCLI_5G_APCTRL rai0 connect -s "$ssid" $APCTRL_BSS_ARG $APCTRL_KEY_ARG "$key"
...
drv_mt7612_setup() {
	...
	json_get_vars main_if channel mode hwmode wmm htmode \
		...
		disabled maxassoc macpolicy maclist noscan smart #device所有配置项
	json_select ..
