...
detect_mt7612() {
	...
	cd /sys/module/
		[ -d mt76x2e ] || [ -d mt76x2_ap ] || [ -d mt7610e ] || return 0
	...
	[ -f /etc/Wireless/iNIC/iNIC_ap.dat ] || {
		mkdir -p /etc/Wireless/iNIC/ 2>/dev/null
		touch $CFG_FILES_2ND
		ln -s $CFG_FILES_2ND /etc/Wireless/iNIC/iNIC_ap.dat 2>/dev/null
	}
...
config wifi-iface
	...
	option wps	none
# 	option doth     1
	option ssid     PandoraBox_5G_$(echo $(rt2860v2_get_second_if_mac Factory) | awk -F ":" '{print $4""$5""$6 }'| tr a-z A-Z)
